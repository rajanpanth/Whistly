...
export default function KickTickMarketCard({ market }: Props) {
  ...
  return (
    <Link href={market.tradeHref} className="group block">
      <article className="flex min-h-[226px] flex-col rounded-[0.9rem] border border-[color:var(--market-border)] bg-[color:var(--market-card)] p-4 transition duration-200 hover:-translate-y-0.5 hover:border-[color:var(--market-border-strong)] hover:bg-[#1e1e23]">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <h3 className="line-clamp-2 text-[15px] font-bold leading-5 text-[color:var(--market-text)]">{market.question}</h3>
            <div className="mt-1 truncate text-xs text-[#8b8b94]">{market.matchName}</div>
          </div>
          <Bookmark size={17} className="shrink-0 text-[color:var(--market-text-3)] transition group-hover:text-white" />
        </div>
        ...
        <div className="mt-4 grid grid-cols-2 gap-2">
          <div className="flex h-10 items-center justify-between rounded-[0.65rem] border border-[#20d38a]/25 bg-[#20d38a]/[0.08] px-4 text-sm font-bold text-[color:var(--market-positive-soft)] transition group-hover:border-[color:var(--market-positive)] group-hover:bg-[color:var(--market-positive)] group-hover:text-[#0a0a0c]"><span>Buy Yes</span><span className="font-mono">{market.yesProbability}%</span></div>
          <div className="flex h-10 items-center justify-between rounded-[0.65rem] border border-[#fa4669]/25 bg-[#fa4669]/[0.08] px-4 text-sm font-bold text-[color:var(--market-live-soft)] transition group-hover:border-[#fa4669]/60"><span>Buy No</span><span className="font-mono">{market.noProbability}%</span></div>
        </div>

        <div className="mt-auto flex items-center gap-2 pt-4 text-xs font-medium text-[#85858e]">
          {live ? <span className="flex items-center gap-1.5 font-bold text-[color:var(--market-live)]"><span className="h-2 w-2 animate-pulse rounded-full bg-current" />LIVE</span> : <span className="font-mono">{resolved ? "RESOLVED" : market.timeLeft}</span>}
          <span>·</span><span className="truncate">{market.volumeSol} Vol.</span><span>·</span><span className="truncate">{market.family}</span>
          <div className="ml-auto flex items-center gap-3"><MessageCircle size={15}/><Gift size={15}/>{live && <Radio size={14} className="text-[color:var(--market-live)]"/>}</div>
        </div>
      </article>
    </Link>
  );
