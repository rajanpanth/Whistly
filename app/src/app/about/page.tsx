...
export default function AboutPage() {
  return (
    <main className="mx-auto max-w-5xl px-4 py-16 sm:px-6">
      <section className="rounded-2xl border border-[#29292f] bg-[#19191d] p-8 text-center sm:p-12">
        ...
        <p className="mt-2 text-sm text-[#6f6f78]">Prediction markets for every moment</p>
        <p className="mx-auto mt-6 max-w-2xl text-base leading-7 text-[#c9c9ce]">Trade every football moment before the clock runs out. Whistly combines fast, easy-to-understand markets with transparent, data-driven settlement.</p>
        <Link href="/events" className="mt-8 inline-flex h-12 items-center gap-2 rounded-full bg-[#f4f4f5] px-6 text-sm font-bold text-[#0a0a0c]">Explore markets <ArrowRight size={16} /></Link>
      </section>

      <section className="py-16">
        <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-[#20d38a]"><Trophy size={16} /> How Whistly works</div>
        <div className="mt-6 grid gap-4 md:grid-cols-3">
          {features.map(({ icon: Icon, title, text }) => (
            <article key={title} className="rounded-2xl border border-[#29292f] bg-[#141418] p-6">
              <Icon className="text-[#20d38a]" size={22} />
