  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6 sm:mb-8">
        <div><div className="text-[11px] font-bold uppercase tracking-[0.16em] text-[#a1a1aa]">Prediction markets</div><h1 className="mt-1 font-heading text-2xl font-bold uppercase tracking-[-0.03em] text-white sm:text-3xl">{t("polls")}</h1></div>
        <Link href="/create" className="px-4 sm:px-6 py-2 sm:py-2.5 bg-[#f4f4f5] text-[#0a0a0c] hover:bg-white rounded-[0.65rem] font-bold transition-all text-sm sm:text-base active:scale-[0.97]">
          {t("createPollPlus")}
        </Link>
      </div>
      ...
      <div className="relative mb-4 sm:mb-5">
        <svg className="absolute left-3 top-1/2 -translate-y-1/2 text-[#6f6f78]" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        <input
          ...
          className="w-full pl-10 pr-4 py-2.5 sm:py-3 bg-[#0c0c0f] border border-[#1c1c20] rounded-[0.65rem] text-sm focus:border-[#3b3b43] focus:bg-[#111114] outline-none transition-all placeholder-[#5d5d65]"
        />
        {search && (
          <button
            onClick={() => handleSearchChange("")}
            aria-label="Clear search"
            className="absolute right-3 top-1/2 -translate-y-1/2 text-[#6f6f78] hover:text-white transition-colors"
          >
      ...
      <div className="space-y-3 sm:space-y-4 mb-6 sm:mb-8">
          ...
          {CATEGORIES.map((cat) => (
            <button
              ...
              className={`px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg text-xs sm:text-sm transition-colors ${selectedCategory === cat
                ? "bg-[#f4f4f5] text-[#0a0a0c] font-bold"
                : "bg-[#111114] text-[#898991] hover:bg-[#18181c] hover:text-white"
                }`}
            >
              {tCat(cat, lang)}
            </button>
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 sm:gap-3">
          ...
          <div className="flex bg-[#111114] border border-[#29292f] rounded-[0.65rem] p-0.5 sm:p-1 gap-0.5" role="radiogroup" aria-label="Sort polls by">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20">
          <div className="w-16 h-16 mx-auto mb-4 rounded-2xl bg-[#141418] border border-[#29292f] flex items-center justify-center">
            <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="text-[#6f6f78]"><circle cx="11" cy="11" r="8" /><line x1="21" y1="21" x2="16.65" y2="16.65" /></svg>
          </div>
          <p className="text-[#a1a1aa] text-lg mb-2 font-medium">{t("noPollsFound")}</p>
          <p className="text-[#6f6f78] text-sm mb-5">{t("noPollsHint")}</p>
          <Link href="/create" className="inline-flex items-center gap-1.5 text-[#d7d7dc] hover:text-white font-medium transition-colors">
        ...
        <>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-2 mt-8">
              <button
                ...
                disabled={page === 1}
                className="px-3 py-1.5 rounded-lg text-sm font-medium transition-colors bg-[#111114] text-[#898991] hover:bg-[#18181c] hover:text-white disabled:opacity-40 disabled:cursor-not-allowed"
                ...
                {Array.from({ length: totalPages }, (_, i) => i + 1)
                  ...
                  .map((p, idx, arr) => (
                    <span key={p} className="contents">
                      {idx > 0 && arr[idx - 1] !== p - 1 && (
                        <span className="px-1 text-[#6f6f78] text-sm">…</span>
                      )}
                      <button
                        onClick={() => setPage(p)}
                        className={`w-8 h-8 rounded-lg text-sm font-medium transition-colors ${p === page ? "bg-[#f4f4f5] text-[#0a0a0c] font-bold" : "bg-[#111114] text-[#898991] hover:bg-[#18181c] hover:text-white"
                          }`}
                      >
                        {p}
                      </button>
          ...
          <p className="text-center text-xs text-[#6f6f78] mt-3">
